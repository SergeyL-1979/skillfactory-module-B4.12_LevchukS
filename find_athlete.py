...
class Athelete(Base):
    ...
    # Заметим, что в БД в таблице с атлетом есть еще много колонок
    # Но мы внутри класса можем определить только те, что нужны нам
...
def main():
    ...
    if user:
        ...
        print("Ближайший по возрасту атлет: {} {}".format(athelete_close_birthdate.name, athelete_close_birthdate.birthdate))
    else:
